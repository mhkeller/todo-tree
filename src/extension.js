...
let refreshTimeout;
let fileRefreshTimeout;
let hideTimeout;
let autoGitRefreshTimer;
let periodicRefreshTimer;
const lastGitHead = {};
const openDocuments = {};
let provider;
...
function activate (context) {
	...
	let buildCounter = context.workspaceState.get('buildCounter', 1);
	...
	function updateInformation () {
		const statusBar = vscode.workspace.getConfiguration('todo-tree.general').statusBar;

		let counts = provider.getTagCountsForActivityBar();
		let total = Object.values(counts).reduce(function (a, b) { return a + b; }, 0);
		...
		const match = countRegex.exec(todoTreeView.title);
		if (match !== null) {
			let title;
		...
		if (statusBar === STATUS_BAR_TOTAL) {
			...
			sortedTags.map(function (tag) {
				if (counts[tag] > 0) {
					...
					let icon = attributes.getIcon(tag);
	...
	function onStatusBarClicked () {
		if (config.clickingStatusBarShouldRevealTree()) {
			...
			let setting = vscode.workspace.getConfiguration('todo-tree.general').statusBar;
	...
	function getOptions (filename) {
		...
		const tempIncludeGlobs = context.workspaceState.get('includeGlobs') || [];
		const tempExcludeGlobs = context.workspaceState.get('excludeGlobs') || [];
		const submoduleExcludeGlobs = context.workspaceState.get('submoduleExcludeGlobs') || [];
		...
		const globs = c.get('filtering.passGlobsToRipgrep') === true
	...
	function searchWorkspaces (searchList) {
		...
		if (scanMode === SCAN_MODE_WORKSPACE_AND_OPEN_FILES || scanMode === SCAN_MODE_WORKSPACE_ONLY) {
			const includes = vscode.workspace.getConfiguration('todo-tree.filtering').get('includedWorkspaces', []);
			const excludes = vscode.workspace.getConfiguration('todo-tree.filtering').get('excludedWorkspaces', []);
	...
	function applyGlobs () {
		const includeGlobs = vscode.workspace.getConfiguration('todo-tree.filtering').get('includeGlobs');
		const excludeGlobs = vscode.workspace.getConfiguration('todo-tree.filtering').get('excludeGlobs');
	...
	function getRootFolders () {
		let rootFolders = [];
		let valid = true;
		const rootFolder = vscode.workspace.getConfiguration('todo-tree.general').get('rootFolder');
	...
	function resetGitWatcher () {
		...
		const timerInterval = vscode.workspace.getConfiguration('todo-tree.general').get('automaticGitRefreshInterval');
	...
	function resetPeriodicRefresh () {
		const timerInterval = vscode.workspace.getConfiguration('todo-tree.general').get('periodicRefreshInterval');
	...
	function setButtonsAndContext () {
		...
		const includeGlobs = context.workspaceState.get('includeGlobs') || [];
		const excludeGlobs = context.workspaceState.get('excludeGlobs') || [];
		const hasSubTags = provider.hasSubTags();

		const showRevealButton = c.get('tree.buttons').reveal === true;
		const showScanModeButton = c.get('tree.buttons').scanMode === true;
		const showViewStyleButton = c.get('tree.buttons').viewStyle === true;
		const showGroupByTagButton = c.get('tree.buttons').groupByTag === true;
		const showGroupBySubTagButton = c.get('tree.buttons').groupBySubTag === true;
		const showFilterButton = c.get('tree.buttons').filter === true;
		const showRefreshButton = c.get('tree.buttons').refresh === true;
		const showExpandButton = c.get('tree.buttons').expand === true;
		const showExportButton = c.get('tree.buttons').export === true;
	...
	function isIncluded (uri) {
		if (uri.fsPath) {
			const includeGlobs = vscode.workspace.getConfiguration('todo-tree.filtering').get('includeGlobs');
			let excludeGlobs = vscode.workspace.getConfiguration('todo-tree.filtering').get('excludeGlobs');
			const includeHiddenFiles = vscode.workspace.getConfiguration('todo-tree.filtering').get('includeHiddenFiles');
			...
			const included = utils.isIncluded(uri.fsPath, includeGlobs.concat(tempIncludeGlobs), excludeGlobs.concat(tempExcludeGlobs));
	...
	function refreshFile (document) {
		...
		if (config.isValidScheme(document.uri) && isIncluded(document.uri) === true) {
			if (config.scanMode() !== SCAN_MODE_CURRENT_FILE || (vscode.window.activeTextEditor && document.fileName === vscode.window.activeTextEditor.document.fileName)) {
				const extractExtraLines = function (section) {
	...
	function addTag (tag) {
		const tags = vscode.workspace.getConfiguration('todo-tree.general').get('tags');
	...
	function removeTagDialog () {
		let tags = vscode.workspace.getConfiguration('todo-tree.general').get('tags');
	...
	function checkForMarkdownUpgrade () {
		if (markdownUpdatePopupOpen === false && ignoreMarkdownUpdate === false) {
			...
			if (c.get('regex.regex').indexOf('|^\\s*- \\[ \\])') > -1) {
				...
				const buttons = [MORE_INFO_BUTTON, NEVER_SHOW_AGAIN_BUTTON];
	...
	function register () {
		function migrateSettings () {
			...
			if (vscode.workspace.getConfiguration('todo-tree.highlights').schemes !== undefined) {
				...
				if (currentSchemes !== schemesSettings.defaultValue) {
					const target = settingLocation('highlights.schemes');
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.exportTree', function () {
			let exportPath = vscode.workspace.getConfiguration('todo-tree.general').get('exportPath');
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.showOnlyThisFolder', function (node) {
			const rootNode = tree.locateWorkspaceNode(node.fsPath);
			const includeGlobs = [utils.createFolderGlob(node.fsPath, rootNode.fsPath, '/*')];
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.switchScope', function () {
			const config = vscode.workspace.getConfiguration('todo-tree.filtering').get('scopes');
			...
			if (!config || config.length === 0) {
				...
				config.forEach(function (c) {
					const scope = { label: c.name };
					const includeGlobs = JSON.stringify(utils.toGlobArray(c.includeGlobs));
					const excludeGlobs = JSON.stringify(utils.toGlobArray(c.excludeGlobs));
				...
				const options = { placeHolder: 'Select scope...' };
				vscode.window.showQuickPick(items, options).then(function (scope) {
					if (scope) {
						const currentConfig = config.find(c => c.name === scope.label);
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.excludeThisFolder', function (node) {
			const rootNode = tree.locateWorkspaceNode(node.fsPath);
			const glob = utils.createFolderGlob(node.fsPath, rootNode.fsPath, '/**/*');
			const excludeGlobs = context.workspaceState.get('excludeGlobs') || [];
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.removeFilter', function () {
			...
			let excludeGlobs = context.workspaceState.get('excludeGlobs') || [];
			let includeGlobs = context.workspaceState.get('includeGlobs') || [];
			const choices = [];
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.goToNext', function () {
			...
			editor.selections.map(function (selection) {
				...
				if (matches && matches.length) {
					...
					const newPosition = editor.document.positionAt(offset);
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.goToPrevious', function () {
			...
			editor.selections.map(function (selection) {
				...
				if (lastMatchOffset !== -1) {
					...
					const newPosition = editor.document.positionAt(lastMatchOffset);
					newSelections.push(new vscode.Selection(newPosition, newPosition));
				} else {
					ok = false;
				}
		...
		context.subscriptions.push(vscode.commands.registerCommand('todo-tree.revealInFile', function (uri, selection) {
			function flashLine () {
				...
				const flashBackgroundColour = new vscode.ThemeColor('editor.rangeHighlightBackground');
				...
				const lineFlashStyle = vscode.window.createTextEditorDecorationType(decorationOptions);

				const lineRangeHighlight = { range: currentLineRange };
			...
			if (vscode.workspace.getConfiguration('todo-tree.tree').autoRefresh === true && config.scanMode() !== SCAN_MODE_WORKSPACE_ONLY) {
				if (config.isValidScheme(document.uri)) {
					if (config.scanMode() !== SCAN_MODE_WORKSPACE_AND_OPEN_FILES) {
						...
						let keep = false;
						const tempSearchList = getRootFolders();
