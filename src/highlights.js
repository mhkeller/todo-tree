...
function getDecoration (tag) {
	...
	if (lane !== undefined) {
		let rulerColour = getRulerColour(tag, darkBackgroundColour || 'editor.foreground');
			...
			while ((match = regex.exec(text)) !== null) {
				...
				if (type !== 'none') {
					...
					if (type === 'text-and-comment') {
						...
						const endOfLineOffset = editor.document.offsetAt(new vscode.Position(fullEndPos.line, editor.document.lineAt(fullEndPos.line).range.end.character));
						...
						if (subTagMatch !== null && subTagMatch.length > 1) {
							...
							if (customHighlight[subTag] !== undefined) {
								...
								if (subTagOffset !== -1) {
									...
									const subTagEndPos = editor.document.positionAt(offsetEnd + subTagOffset + subTagMatch[1].length);
