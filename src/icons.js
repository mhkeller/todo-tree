...
const codiconNames = require('./codiconNames.js');
const productIconNames = require('./productIconNames.js');

function getIcon (context, tag, debug) {
	let colour = attributes.getIconColour(tag);

	let darkIconPath = context.asAbsolutePath(path.join('resources/icons', 'dark', 'todo-green.svg'));
	let lightIconPath = context.asAbsolutePath(path.join('resources/icons', 'light', 'todo-green.svg'));

	let compactColourName = colour;
	compactColourName = compactColourName.replace(/[^0-9a-zA-Z]/g, '');

	let iconName = attributes.getIcon(tag);
	...
	if (iconName === 'todo-tree' || iconName === 'todo-tree-filled') {
		...
		const colouredTodoTreeIconPath = path.join(context.globalStorageUri.fsPath, iconName + '-' + compactColourName + '.svg');
		if (!fs.existsSync(colouredTodoTreeIconPath)) {
			const colouredTodoTreeIconDefinition = (iconName === 'todo-tree')
		...
		if (context.globalStorageUri.fsPath) {
			const octiconIconPath = path.join(context.globalStorageUri.fsPath, 'todo-' + iconName + '-' + compactColourName + '.svg');
			if (!fs.existsSync(octiconIconPath)) {
				const octiconIconDefinition = '<?xml version="1.0" encoding="iso-8859-1"?>\n' +
		...
		const colouredIconPath = path.join(context.globalStorageUri.fsPath, 'todo-' + compactColourName + '.svg');
		if (!fs.existsSync(colouredIconPath)) {
			const colouredIconDefinition =
	...
	function checkIcon (setting) {
		...
		if (icon !== undefined) {
			if (utils.isCodicon(icon)) {
				const codicon = icon.substr(2, icon.length - 3);
