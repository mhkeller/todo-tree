...
function tags () {
	const tags = vscode.workspace.getConfiguration('todo-tree.general').tags;
...
function isValidScheme (uri) {
	const schemes = vscode.workspace.getConfiguration('todo-tree.general').schemes;
	return uri && uri.scheme && schemes && schemes.length && schemes.indexOf(uri.scheme) !== -1;
}

function shouldUseBuiltInFileExcludes () {
	const useBuiltInExcludes = vscode.workspace.getConfiguration('todo-tree.filtering').useBuiltInExcludes;
...
function refreshTagGroupLookup () {
	const tagGroups = vscode.workspace.getConfiguration('todo-tree.general').tagGroups;
